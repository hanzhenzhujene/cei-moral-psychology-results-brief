...
| Failure recovery | Fourteen slide-release tests cover every replacement point, public-validation failure, real `SIGINT`, real `SIGTERM`, both cleanup paths, and truthful post-interrupt status messages. One source-snapshot test covers a builder replacement during Python startup. The public hashes and signal state are restored before an in-transaction interrupt propagates |
